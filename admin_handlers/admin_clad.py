...
async def add_clad(call: CallbackQuery, bot: Bot, state: FSMContext):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT name_city FROM city")
        ...
        if len(items) == 0:
            ...
            for i in range(0, len(items), 2):
                ...
                if i < len(items) - 1:
                    row.append(InlineKeyboardButton(text=items[i][0],
                                                    callback_data=f'_clad_city_{items[i][0]}'))
                    row.append(
                        InlineKeyboardButton(text=items[i + 1][0],
                                             callback_data=f'_clad_city_{items[i + 1][0]}'))
                else:
                    row.append(InlineKeyboardButton(text=items[i][0],
                                                    callback_data=f'_clad_city_{items[i][0]}'))
            ...
            await bot.send_message(user_id, f"Выберите Город для добавление клада:", reply_markup=key_city)
            await bot.delete_message(user_id, call.message.message_id)
    except:
        pass
...
async def add_clad_area(call: CallbackQuery, bot: Bot, state: FSMContext):
    try:
        ...
        conn = sqlite3.connect('shop.db')
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT name_area FROM area WHERE name_city = ?", (city,))
        items = cursor.fetchall()
        conn.close()
        ...
        if len(items) == 0:
            ...
            for i in range(0, len(items), 2):
                ...
                if i < len(items) - 1:
                    row.append(InlineKeyboardButton(text=items[i][0],
                                                    callback_data=f'_clad_area_{city}_{items[i][0]}'))
                    row.append(
                        InlineKeyboardButton(text=items[i + 1][0],
                                             callback_data=f'_clad_area_{city}_{items[i + 1][0]}'))
                else:
                    row.append(InlineKeyboardButton(text=items[i][0],
                                                    callback_data=f'_clad_area_{city}_{items[i][0]}'))
                key_city.inline_keyboard.append(row)
            ...
            await bot.send_message(user_id, f"Выберите Район для добавление клада:", reply_markup=key_city)
            await bot.delete_message(user_id, call.message.message_id)
    except:
        pass
    ...
    try:
        ...
        if len(items) == 0:
            ...
            for i in range(0, len(items), 2):
                ...
                if i < len(items) - 1:
                    row.append(InlineKeyboardButton(text=items[i][0],
                                                    callback_data=f'_clad_item_{city}_{area}_{items[i][0]}'))
                    row.append(
                        InlineKeyboardButton(text=items[i + 1][0],
                                             callback_data=f'_clad_item_{city}_{area}_{items[i + 1][0]}'))
                else:
                    row.append(InlineKeyboardButton(text=items[i][0],
                                                    callback_data=f'_clad_item_{city}_{area}_{items[i][0]}'))
                key_city.inline_keyboard.append(row)
            ...
            await bot.send_message(user_id, f"Выберите Товар для добавление клада:", reply_markup=key_city)
            await bot.delete_message(user_id, call.message.message_id)
    except:
        pass
...
async def show_clad(call: CallbackQuery, bot: Bot):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            'SELECT DISTINCT name_city FROM clad')
        ...
        if len(rows) > 0:
            for i in range(0, len(rows), 2):
                ...
                if i < len(rows) - 1:
                    ...
                    row.append(
                        InlineKeyboardButton(text=rows[i + 1][0], callback_data=f'gram_admin_show_{rows[i + 1][0]}_{item}_{city}'))
                else:
                    row.append(InlineKeyboardButton(text=rows[i][0], callback_data=f'gram_admin_show_{rows[i][0]}_{item}_{city}'))
                key_city.inline_keyboard.append(row)
            key_city.inline_keyboard.append([
    ...
    try:
        ...
        if len(rows) > 0:
            for row in rows:
                gram, count = row
                key_city.inline_keyboard.append([
                    InlineKeyboardButton(text=f'Район {area}\n{gram} грамм - {count} кладов', callback_data=f'end_clad_show_{gram}_{city}_{item}_{area}')
                ])
            key_city.inline_keyboard.append([
