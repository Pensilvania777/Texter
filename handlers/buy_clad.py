...
async def buy_area(call: CallbackQuery, bot: Bot):
    try:
        ...
        conn = sqlite3.connect('shop.db')
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT name_area FROM clad WHERE name_city = ?", (city,))
            ...
            if launge == "ru":
                ...
                keyboard = InlineKeyboardMarkup(inline_keyboard=[
                    [
                        ...
                        InlineKeyboardButton(text=f"{ark[0]} - ({count} шт.) ", callback_data=f'_ars_area_{city}_{area[0]}_{launge}_{item}')
                        for ark in arks
                    ],
                    [
                ...
                keyboard = InlineKeyboardMarkup(inline_keyboard=[
                    [
                        InlineKeyboardButton(text=f"{area[0]} - ({count} am.) ", callback_data=f'_ars_area_{city}_{area[0]}_{launge}_{item}')
                        for area in areas
                    ],
                    [
