                ...
                for i in range(0, len(grams), 2):
                    ...
                    if i < len(grams) - 1:
                        row.append(InlineKeyboardButton(text=grams[i][0],
                                                        callback_data=f'_ars_gram_{city}_{area}_{grams[i][0]}_{launge}_{itemsnow}_{photo_data}'))
                        row.append(
                            InlineKeyboardButton(text=grams[i + 1][0],
                                                 callback_data=f'_ars_gram_{city}_{area}_{grams[i + 1][0]}_{launge}_{itemsnow}_{photo_data}'))
                    else:
                        row.append(InlineKeyboardButton(text=grams[i][0],
                                                        callback_data=f'_ars_gram_{city}_{area}_{grams[i][0]}_{launge}_{itemsnow}_{photo_data}'))
                    key_city.inline_keyboard.append(row)
...
async def buy_item_end(call: CallbackQuery, bot: Bot):
    try:
        ...
        photo_data = call.data.split('_')[8]
